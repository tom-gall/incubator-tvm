...
class Frontend(ABC):
    ...
    @abstractmethod
    def suffixes():
        """File suffixes used by this frontend"""
...
class KerasFrontend(Frontend):
    ...
    def load(self, path, shapes):
        ...
        try:
            logging.debug("loading model %s using Keras frontend.", path)
...
class OnnxFrontend(Frontend):
    ...
    def load(self, path, shapes):
        ...
        logging.debug("input '%s' shape is %s", name, shape)
        shape_dict = {name: shape}

        logging.debug("loading model %s using ONNX frontend.", path)
...
class TensorflowFrontend(Frontend):
    ...
    def load(self, path, shapes):
        ...
        logging.debug("loading model %s using TensorFlow frontend.", path)
...
class TFLiteFrontend(Frontend):
    ...
    def load(self, path, shapes):
        ...
        logging.debug("input '%s' type is %s", shape_dict, dtype_dict)

        # parse TFLite model and convert into Relay computation graph
        logging.debug("loading model %s using TFLite frontend.", path)
...
class PyTorchFrontend(Frontend):
    ...
    def load(self, path, shapes):
        ...
        logging.debug("")
        logging.debug("loading model %s using PyTorch frontend.", path)
...
def load_model(path, language=None, sanitize_diagnostics=True, shapes=None):
    ...
    sanitize_diagnostics : bool
        Whether to convert exceptions to TVMCExceptions.
    ...
    try:
        ...
        if sanitize_diagnostics:
            raise TVMCException("AssertionError: {}".format(err))
        raise err
    except KeyError as err:
        if sanitize_diagnostics:
            raise TVMCException("KeyError: {}".format(err))
        raise err
    except TypeError as err:
        if sanitize_diagnostics:
            raise TVMCException("TypeError: {}".format(err))
        raise err
    # tvm.error.OpNotImplemented is actually derived from
    # NotImplementedError, but keeping the patterns separate here
    # until tvm itself is consistent in using tvm.error.
    except tvm.error.OpNotImplemented as err:
        if sanitize_diagnostics:
            raise TVMCException(str(err))
        raise err
    except NotImplementedError as err:
        if sanitize_diagnostics:
            raise TVMCException("NotImplementedError: {}".format(err))
        raise err
